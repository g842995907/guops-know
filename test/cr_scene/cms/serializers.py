...
class CrSceneSerializer(ModelSerializer):
    ...
    def validate_name(self, value):
        if value == '':
            raise exceptions.ValidationError(error.NAME_NOT_EMPTY)
        if self.instance is None and self.Meta.model.objects.filter(name=value).exists():
            raise exceptions.ValidationError(error.NAME_EXIST(name=value))
        if self.instance and self.Meta.model.objects.filter(name=value).exclude(id=self.instance.id).exists():
            raise exceptions.ValidationError(error.NAME_EXIST(name=value))
        return value
