...
from common_scene.storage.views import StorageAction
...
from common_scene.compute import params as cpt_params
from common_scene.docker import params as ctn_params
from common_scene.utils import LazyLoader
...
IMAGE_FOLDER = project_utils.glance_image_dir() or \
               api_settings.COMPLEX_MISC.get("glance_image_dir")
CPU_RATIO = float(project_utils.get_nova_config("DEFAULT", "cpu_allocation_ratio")
                  or api_settings.COMPLEX_MISC.get("cpu_allocation_ratio") or 4.0)
RAM_RATIO = float(project_utils.get_nova_config("DEFAULT", "ram_allocation_ratio")
                  or api_settings.COMPLEX_MISC.get("ram_allocation_ratio") or 1.0)
DISK_RATIO = float(project_utils.get_nova_config("DEFAULT", "disk_allocation_ratio")
                   or api_settings.COMPLEX_MISC.get("disk_allocation_ratio") or 2.0)
...
class BaseScene(ImageAction, NetworkAction, InstanceAction, ContainerAction, StorageAction):
    ...
    def update_image_cache(self, imgs=None):
        ...
        cache.set(IMAGES_CACHE_KEY, img_dict, 86400)
        return img_dict

    def _get_image_from_cache(self, image_name):
        # get image from memcache
        imgs = cache.get(IMAGES_CACHE_KEY)
    ...
    def update_flavor_cache(self, flavors=None):
        ...
        cache.set(FLAVORS_CACHE_KEY, flavor_dict)

    def _get_flavor_from_cache(self, flavor_name):
        # get image from memcache
        flavors = cache.get(FLAVORS_CACHE_KEY)
        ...
        if not flavor:
            ...
            if os_type and os_type == "windows":
                flavor = self._get_flavor_from_cache(
                    flavor_name=api_settings.COMPLEX_MISC.get("windows_flavor"))
            else:
                flavor = self._get_flavor_from_cache(
                    flavor_name=api_settings.COMPLEX_MISC.get("linux_flavor"))
        return flavor
    ...
    def _get_security_groups(self):
        return api_settings.COMPLEX_MISC.get("security_groups")
    ...
    def fresh_memcache_fips(self):
        ...
        cache.set(AVAILABLE_FIPS_KEY, avail_fips)
        return avail_fips

    def mark_used_fips(self, pre_fips):
        used_fips = cache.get(PREALLOCATED_FIPS_KEY) or {}
        cache.set(PREALLOCATED_FIPS_KEY, dict(used_fips, **pre_fips))
    ...
    def clean_used_fips(self, pre_fips):
        ...
        cache.set(PREALLOCATED_FIPS_KEY, used_fips)

    def _get_fip(self, skip_cache=False):
        if not skip_cache:
            # get from memcache
            avail_fips = cache.get(AVAILABLE_FIPS_KEY) or {}
            pre_fips = cache.get(PREALLOCATED_FIPS_KEY) or {}
    ...
    def preallocate_fips(self, count):
        ...
        with project_utils.memcache_lock(cache, MEMCACHE_LOCKER_KEY):
            # avialable_fips = self.load_available_fips_dict()
            # cache.set(AVAILABLE_FIPS_KEY, avialable_fips)
            #
    ...
    def _calculate_allowance(self):
        ...
        ext_nets = cache.get("_check_ext_networks_")
        if not ext_nets:
            try:
                ext_nets = self.neutron_cli.ext_networks_list()
                cache.set("_check_ext_networks_", ext_nets, 86400)
            except Exception as e:
    ...
    def _calculate_preallocated(self):
        ...
        preallocated_resoureces = cache.get(PREALLOCATED_RESOURCES)
    ...
    def ensure_preallocated(self, locker_id, req_vcpus,
        ...
        preallocated_resoureces = cache.get(PREALLOCATED_RESOURCES) or {}
        ...
        cache.set(PREALLOCATED_RESOURCES, preallocated_resoureces, 3600)
    ...
    def scene_delete_firewall_rules(self, firewall_id, **kwargs):
        ...
        if direction in ['ingress', 'egress']:
            ...
            for orig_rule in rules:
                if isinstance(orig_rule, basestring):
    ...
    def _get_container_userdata(self, attach_url, os_type=None, users=None, run=True,
        ...
        if attach_url:
            ...
            # execute install.sh
            if install_script:
                if install_script.startswith("./") or install_script.startswith("/"):
            ...
            # execute init.sh
            if init_script:
                if init_script.startswith("./") or init_script.startswith("/"):
    ...
    def scene_send_create_container(self, **kwargs):
        ...
        run = kwargs.get("run", True)
        command = init_script or ""
        if attach_url and init_script:
            commands = self._get_container_userdata(
                attach_url, os_type, users, run,
                custom_script=kwargs.get("custom_script"),
                install_script=kwargs.get("install_script"),
    ...
    def scene_check_create_container(self, container, **kwargs):
        ...
        floating_ip = kwargs.get("floating_ip")
        if floating_ip and container_obj.addresses:
            port = "{}_{}".format(container_obj.addresses.values()[0][0].get('port'),
                                  container_obj.addresses.values()[0][0].get('addr'))
            fip_obj = self.bind_fip(floating_ip, port=port)
            setattr(container_obj, "floating_ip", fip_obj.get("floating_ip_address"))
    ...
    def check_first_boot(self, instance_id=None, image=None):
        if instance_id:
            ...
            try:
                ...
                err_msg = _("Unable to check first boot.")
                self._handle_error(err_msg, e)
    ...
    def clear_unused_port(self):
        from datetime import datetime, timedelta

        search_params = {'status': "DOWN"}
        ext_net_id = api_settings.COMPLEX_MISC.get("external_net")
        if ext_net_id:
            search_params.update({"network_id": ext_net_id})
        ports = self.neutron_cli.port_list(**search_params)
        for port in ports:
            created_at = port.get("created_at")
            try:
                if datetime.now() - datetime.strptime(created_at, "%Y-%m-%dT%H:%M:%SZ") > timedelta(hours=2):
                    self.delete_port(port.get("id"))
                    LOG.debug("[OPENSTACK] Clear unused "
                              "port ({})".format(port.get("id")))
            except Exception as e:
                LOG.error("[OPENSTACK] Unable to clear "
                          "unused port ({}). {}".format(port.get("id"), getattr(e, "message")))

    def sync_docker_image(self, image):
        if not IMAGE_FOLDER:
            err_msg = _("Image store data directory not "
                        "configured or not exists.")
            self._handle_error(err_msg)
        ...
        hosts = self.zun_cli.service_list()
        for host in hosts:
            if host.binary != "zun-compute":
                continue
            if host.state != "up":
                continue

            LOG.debug("Sync docker image ({}) on "
                      "host {}".format(image_name, host.host))
            base_url = "tcp://{}:2375".format(host.host)
            dock_cli = docker_client.Client(base_url=base_url)

            try:
                dock_cli.load_image(image_path)
                LOG.info("Loaded docker image {} "
                         "on host {}".format(image_name, host.host))
            except Exception, e:
                err_msg = _("Unable to load docker image {} "
                            "on host {}".format(image_name, host.host))
                self._handle_error(err_msg, e)

    def rename_docker_image(self, orig_name, new_name):
        hosts = self.zun_cli.service_list()
        for host in hosts:
            if host.binary != "zun-compute":
                continue
            ...
            LOG.debug("Rename docker image ({}) --> ({}) on "
                      "host {}".format(orig_name, new_name, host.host))
            base_url = "tcp://{}:2375".format(host.host)
            dock_cli = docker_client.Client(base_url=base_url)
            try:
                orig_image = dock_cli.inspect_image(orig_name)
            except Exception, e:
                LOG.error("Unable to ger image ({}) from docker "
                          "on host {}".format(orig_name, host.host))
                continue

            # rename docker image
            try:
                dock_cli.tag_image(orig_image, new_name)
                # new_image = dock_cli.inspect_image(new_name)
            except Exception, e:
                LOG.error("Failed to rename image ({}) --> ({}) "
                          "on host {}".format(orig_name, new_name, host.host))
                continue

            try:
                dock_cli.remove_image(orig_name)
            except Exception, e:
                LOG.error("Failed to remove origin docker "
                          "image ({}) on host {}".format(orig_image, host.host))
