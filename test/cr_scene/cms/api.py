...
from base.utils.rest.mixins import CacheModelMixin, DestroyModelMixin, PMixin, RecordMixin
from base.utils.response import TmpFileResponse
from base.utils.error import stack_error
from base.utils.request import RequestTmpDiskFile
...
from cr_scene.utils.resource import export_cr_scenes, import_cr_scenes
...
from .error import error, mission_period_error

logger = logging.getLogger(__name__)


class CrSceneViewSet(RecordMixin,
                     BatchSetOwnerModelMixin,
                     DestroyModelMixin,
                     CacheModelMixin,
                     PMixin,
    ...
    def batch_export(self, request):
        ids = self.query_data.getlist('ids', int)
        if not ids:
            return Response(status=status.HTTP_200_OK)

        cr_scenes = CrScene.objects.filter(pk__in=ids)
        try:
            file_path = export_cr_scenes(cr_scenes)
        except Exception as e:
            raise exceptions.APIException(e.message)

        return TmpFileResponse(file_path)
    ...
    def batch_import(self, request):
        ...
        for f in files:
            try:
                with RequestTmpDiskFile(f) as file_path:
                    import_cr_scenes(file_path)
            except Exception as e:
                logger.error('import cr_scene error: %s', e)
                stack_error()
                raise exceptions.APIException(error.INVALID_PARAMS)
    ...
    if scene_id:
        if status == Scene.Status.RUNNING:
            if event == SceneStatusUpdateEvent.SCENE_CREATE:
                ...
                # 流量发生开始
                SceneTrafficManager(scene_id, cr_scene_id=cr_scene_id).start_traffic_event()
            ...
            # 删除流量
            SceneTrafficManager(scene_id, cr_scene_id=cr_scene_id).stop_traffic_event()
    ...
    def bulk_create_update(self, request):
        ...
        if cr_scene_id is not None:
            try:
                ...
                raise exceptions.ValidationError(mission_period_error.INVALID_SCENE_ID)
        else:
            raise exceptions.ValidationError(mission_period_error.SCENE_ID_REQUIRED)
        periods = data.get('periods')

        if periods is None:
            raise exceptions.ValidationError(mission_period_error.PERIODS_REQUIRED)

        if type(periods) != list:
            raise exceptions.ValidationError(mission_period_error.PERIOD_TYPE_ERROR)

        if len(set(periods)) != len(periods):
            raise exceptions.ValidationError(mission_period_error.PERIOD_DUPLICATE)

        exist_missions = cr_scene.missions
        if exist_missions.count() > 0:
            min_length_of_period = max(mission.period for mission in exist_missions.all())
        else:
            min_length_of_period = 0
        if len(periods) < min_length_of_period:
            raise exceptions.ValidationError(mission_period_error.PERIOD_DELETE_ERROR)
