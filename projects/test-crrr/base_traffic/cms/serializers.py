    ...
    class Meta:
        ...
        extra_kwargs = {
            'en_name': {'min_length': 2,
                        'max_length': 20,
                        'validators': [UniqueValidator(queryset=TrafficCategory.objects.all(),
...
class IntelligentTrafficSerializer(TrafficSerializer):
    ...
    def to_internal_value(self, data):
        try:
            suffix = int(data.get('suffix')) if data.get('suffix', None) else self.instance.suffix
        except Exception:
            raise exceptions.ValidationError({'suffix': _('x_loss_suffix')})

        code = data.get('code', '')
        if suffix == IntelligentTraffic.Suffix.PY and 'traffic' not in code:
            raise exceptions.ValidationError({'code': _('x_traffic_code_error')})

        ret = super(IntelligentTrafficSerializer, self).to_internal_value(data)
        return ret
