...
def cms_task_list(request):
    tasks = practice_api.get_task_list(practice_api.PRACTICE_TYPE_THEORY)
    logger.info(len(tasks))

    return render(request, 'practice/cms/event_list.html')


@web_login_required
@find_menu(slug)
def task_list(request, event_id, **kwargs):
    type_id = practice_api.PRACTICE_TYPE_THEORY
    event_id = event_id
    context = {
