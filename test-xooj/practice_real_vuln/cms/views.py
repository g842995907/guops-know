...
from practice.api import get_task_event_by_type, PRACTICE_TYPE_REAL_VULN
from practice.constant import TaskEventStatus
from practice.models import TaskEvent
from practice_real_vuln.cms.serializers import PracticeExerciseTaskSerializerForEdit
from practice_real_vuln import models as task_models

render = AppRender('practice_real_vuln', 'cms').render
...
def task_list(request):
    if request.user.is_superuser:
        event_list = TaskEvent.objects.filter(type=int(PRACTICE_TYPE_REAL_VULN))
    else:
        event_list = TaskEvent.objects.filter(type=int(PRACTICE_TYPE_REAL_VULN)).filter(
...
def task_detail(request, pk):
    if request.user.is_superuser:
        ...
        event_list = TaskEvent.objects.filter(type=int(PRACTICE_TYPE_REAL_VULN)).filter(
            Q(share_teachers__in=[request.user.id]) | Q(create_user_id=request.user.id)).distinct()
    category_list = task_models.RealVulnCategory.objects.all()
    context = {
        'category_list': category_list,
        'event_list': event_list
...
def category_detail(request, category_id):
    category = task_models.RealVulnCategory.objects.get(id=category_id)
    return render(request, 'category_detail.html',
                  {"category": category})
