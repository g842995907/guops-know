...
def manual_traffic(data, manual=True):
    traffic_event = TrafficEvent.objects.get(pk=data.get('traffic_event_id'))

    if traffic_event.type == TrafficEvent.Type.BACKGROUND:
        traffic = BackgroundTrafficManager(data, traffic_event, manual)
    else:
        traffic = IntelligentTrafficManager(data, traffic_event, manual)
