...
from common_scene.clients.cinder_client import Client as cnd_client
from common_scene.utils import LazyLoader, logger_decorator, handle_error
...
class StorageAction(object):
    ...
    def delete_volume(self, volume_id, cascade=False, sync=False):
        try:
            ...
            LOG.info("Volume ({}) already deleted.".format(volume_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get volume ({}). \n{}".format(volume_id, e))
                attempts -= 1
                time.sleep(1)
        return True
    ...
    def delete_volume_snapshot(self, snap_id, force=True, is_async=True):
        try:
            ...
            LOG.info("Volume snapshot ({}) already deleted.".format(snap_id))
            return True
        except Exception as e:
        ...
        if not is_async:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get volume snapshot ({}). \n{}".format(snap_id, e))
                    break
                time.sleep(1)
