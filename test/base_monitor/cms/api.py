...
from base.utils.rest.mixins import DestroyModelMixin, CacheModelMixin, PMixin, RecordMixin
...
class MonitorCategoryViewSet(DestroyModelMixin, CacheModelMixin, PMixin, viewsets.ModelViewSet):
    ...
    def perform_batch_destroy(self, queryset):
        for instance in queryset:
            if monitor_models.Scripts.objects.filter(category_id=instance.id):
                raise exceptions.ValidationError(MonitorError.MONITOR_CATEGORY_BE_USED)

        if queryset.update(status=monitor_models.MonitorCategory.Status.DELETE) > 0:
            return True
        return False


class ScriptViewSet(RecordMixin, DestroyModelMixin, CacheModelMixin, PMixin, viewsets.ModelViewSet):
