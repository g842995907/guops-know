...
class MissionCheckerManager(object):
    def __init__(self, mission, cr_scene, scene_id):
        ...
        self.logger = SceneLogFactory(self.scene_id, __name__) if scene_id else logger_critical
...
class BaseChecked(object):
    ...
    def run(self):
        self.logger.info("Mission[%s]: Start check process", self.content.get("title"))
        # 检测参数
...
class RpcChecked(BaseChecked):
    ...
    def _run(self, script, script_client, **kwargs):
        ...
        self.logger.info("Mission[%s]: Check result: %s", self.content.get("title"), _ret)
        ...
        if mission_connect_number <= MAX_BUMBER_ATTEMPTS:
            if _ret.get("status") == "down":
                if mission_connect_number % 9 == 0 or mission_connect_number == 1:
                    self.logger.info("Mission[%s] : RPC client failed,  Reconnecting",
                                     self.content.get("title"))
                start_checker(self.content, self.logger, True)
            else:
                self.logger.info("Mission[%s] : RPC client successful,  Reconnecting",
                                 self.content.get("title"))
                ...
                if mission_status != MissionStatus.STOP:
                    self.logger.debug("Mission[%s]: Check completed, processing result",
                                      self.content.get("title"))
    ...
    def _run(self, script, script_client, **kwargs):
        ...
        if is_run:
            ...
            if mission_connect_number <= MAX_BUMBER_ATTEMPTS:
                ...
                mission_status = mission.mission_status
                if mission_status != MissionStatus.STOP:
                    async_exe(start_checker, (self.content, self.logger), delay=1)
            else:
                record_status_information(
                    self.content.get("id"),
