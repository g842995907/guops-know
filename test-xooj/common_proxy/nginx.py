...
config_template = 'upstream {name} {{server {ip}:{port};}}server {{listen {proxy_port};proxy_connect_timeout {connect_timeout}s;proxy_timeout {timeout}s;proxy_pass {name};}} #{data}'
...
def get_config_path(ip, port):
    return os.path.join(api_settings.NGX_CONF_PATH, config_filename_template.format(ip=ip, port=port))
...
def add_proxy(ip, port, proxy_port=None, timeout=300, connect_timeout=30, name=None, detail=False):
    ...
    if os.path.exists(config_path):
        ...
        if not config or (proxy_port and config['proxy_port'] != proxy_port):
            ...
            return add_proxy(ip, port, proxy_port=proxy_port, timeout=timeout, connect_timeout=connect_timeout, name=name)
    else:
        config = generate_config(ip, port, proxy_port=proxy_port, timeout=timeout, connect_timeout=connect_timeout, name=name)
...
def get_new_valid_port():
    port = random.randint(api_settings.PROXY_START_PORT,api_settings.PROXY_END_PORT)
...
def restart_nginx():
    ...
    if nginxPid is None:
        subprocess.Popen(api_settings.NGX_START_CMD, shell=True)
    else:
        subprocess.Popen(api_settings.NGX_REBOOT_CMD, shell=True)
