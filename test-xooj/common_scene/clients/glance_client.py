...
try:
    ...
    pass
from common_scene.utils import get_ip_by_hostname, is_uuid_like, get_local_hostname
...
class Client(object):
    ...
    def get_ssh_conn(self, hostname="controller"):
        ...
        return ssh(host_ip,
                   api_settings.CONTROLLER_INFO.get("ssh_port") or 22,
                   api_settings.CONTROLLER_INFO.get("ssh_username") or "root",
                   api_settings.CONTROLLER_INFO.get("ssh_password"),
                   api_settings.CONTROLLER_INFO.get("ssh_key_path"))

    def is_image_file_exists(self, file_name, ftp_path=None):
        if not ftp_path:
            ftp_path = api_settings.COMPLEX_MISC.get("ftp_path")
    ...
    def image_create_by_ftp(self, name, file_name, ftp_path=None,
        ...
        command = 'export OS_USERNAME={username} && ' \
                  ...
                  '"{image_name}" >> /var/log/ftp_create_image.log 2>&1'.format(
                             username=api_settings.OS_AUTH.get("username"),
                             password=api_settings.OS_AUTH.get("password"),
                             project=api_settings.OS_AUTH.get("project_name"),
                             user_domain=api_settings.OS_AUTH.get("user_domain_id"),
                             project_domain=api_settings.OS_AUTH.get("project_domain_id"),
                             auth_url=api_settings.OS_AUTH.get("auth_url"),
    ...
    def docker_image_load(self, image_id):
        image_folder = api_settings.COMPLEX_MISC.get("glance_image_dir")\
                       or "/var/lib/glance/images/"
        img_path = os.path.join(image_folder, image_id)
        if not os.path.exists(img_path):
            return None, "Image not found ({})".format(img_path)

        load_cmd = "docker load < {}".format(img_path)
        load_img_proc = subprocess.Popen(load_cmd, shell=True,
                                         stdout=subprocess.PIPE,
                                         stderr=subprocess.PIPE)
        out, err = load_img_proc.communicate()
        return out, err
...
if __name__ == "__main__":
    ...
    # f = open("/home/moose/Downloads/cirros-0.3.5-x86_64-disk.img")
    # data = {'name': str(uuid.uuid4()),
    #         'disk_format': 'qcow2',
    #         'visibility': 'public',
    #         'minimum_disk': 1,
    #         'minimum_ram': 512,
    #         'image_file': UploadedFile(file=f)
    #         }
    # meta = cli.create_image_metadata(data)
    #
    ...
    # img = cli.image_create(**meta)
    # print img

    # Download image
    img_id = 'ca89591b-424f-469b-a20c-cab71ef42e64'
    cc = cli.image_download(img_id)
    with open("/tmp/%s"%img_id, 'wb') as f:
        for c in cc:
            f.write(c)
