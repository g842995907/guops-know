...
from practice_infiltration.response import TaskResError
from . import serializers as mserializers


class PracticeInfiltrationTaskViewSet(
    common_mixins.RecordMixin,
    CacheModelMixin, DestroyModelMixin,
    RequestDataMixin, PublicModelMixin,
    PublicWriteModelMixin, viewsets.ModelViewSet
):
    queryset = infiltration_models.PracticeInfiltrationTask.objects.all()
    serializer_class = mserializers.PracticeInfiltrationTaskSerializer
    ...
    def sub_perform_create(self, serializer):
        ...
        if infiltration_models.PracticeInfiltrationTask.objects.filter(title=serializer.validated_data['title'],
    ...
    def step_by_step(self, serializer):
        ...
        for scores in self.request.data.getlist("score"):
            if not int(scores):
                raise exceptions.ValidationError(TaskResError.SCORE_TO_SMALL)
        score_text = "|".join(self.request.data.getlist("score"))
        return score_text, all_scores


class PracticeInfiltrationCategoryViewSet(CacheModelMixin, DestroyModelMixin,
                                      viewsets.ModelViewSet):
    queryset = infiltration_models.PracticeInfiltrationCategory.objects.all()
    serializer_class = mserializers.PracticeInfiltrationCategorySerializer
    permission_classes = (IsAuthenticated, IsStaffPermission,)
    ...
    def perform_batch_destroy(self, queryset):
        ...
        has_normal_choiceTask = infiltration_models.PracticeInfiltrationTask.original_objects.filter(category__in=ids,
                                                                                                     is_copy=False,
    ...
    def sub_perform_create(self, serializer):
        if infiltration_models.PracticeInfiltrationCategory.objects.filter(
