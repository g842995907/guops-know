...
OS_PROJECT_ID = 'openstack_default_project_id'
...
class Client(object):
    ...
    def _get_tenant_id(self):
        project_id = api_settings.OS_AUTH.get("project_id")
        if project_id:
            return project_id

        project_id = cache.get(OS_PROJECT_ID)
        if not project_id:
            from common_scene.clients import keystone_client
            ks_client = keystone_client.Client()
            project = ks_client.project_get(api_settings.OS_AUTH.get("project_name"))
            if project:
                project_id = project.id
                cache.set(OS_PROJECT_ID, project_id)
    ...
    def ad_sbunets_get(self, prefix):
        subnets = self.subnet_get_all(network_id=api_settings.COMPLEX_MISC.get("ad_net")[0])
    ...
    def floating_ip_allocate(self, pool, tenant_id=None, **params):
        if not tenant_id:
            tenant_id = api_settings.OS_AUTH.get("project_name")
        create_dict = {
...
if __name__ == "__main__":
    ...
    try:
        cli.floating_ip_release("14f32aab-85d2-44dd-9b9c-faa8f459173f")
    except Exception, e:
        pass
    ...
    #
    # qprin = cli.qos_bandwidth_limit_rule_create(policy=qp.get("id"), max_kbps=300, max_burst_kbps=300, direction="ingress")
    # qpre = cli.qos_bandwidth_limit_rule_create(policy=qp.get("id"), max_kbps=200, max_burst_kbps=200, direction="egress")
    #
