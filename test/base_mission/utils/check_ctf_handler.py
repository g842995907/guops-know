...
def save_ctf_or_check_mission(mission, data, update=False):
    ...
    if not update:
        ...
        if mission.type == constant.Type.CTF:
            ...
            script_type = Scripts.Type.REMOTE if (
                        submit_data.get('check_type') == CheckMission.CheckType.SYSTEM) else Scripts.Type.LOCAL
            submit_data["script_id"] = Scripts.get_script_id(submit_data.get('scripts'), script_type)
