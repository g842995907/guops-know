...
from common_remote.utils.guacamole import GuacamoleDatabase, GuacamoleServer, GuacamoleConsumer
...
class RemoteManager(object):
    ...
    def convert_recording(self, recording_name, screen_size='1366x768', callback_script=''):
        return self.guacamole_server.convert_recording(recording_name, screen_size=screen_size, callback_script=callback_script)
...
fake_guacadmin = User(
    id=api_settings.GUACADMIN_USER['id'],
    username=api_settings.GUACADMIN_USER['username'],
    password=api_settings.GUACADMIN_USER['password'],
)
...
if [ ! -d "{recording_dir}" ]; then
  mkdir -p {recording_dir}
    ...
    '''.format(
        guacdrive_dir=api_settings.GUACDRIVE_PATH,
        recording_dir=api_settings.RECORDING_SOURCE_PATH,
    )
    ...
    for server in api_settings.GUACAMOLE_SERVERS:
        try:
            ...
            logger.error('check guacamole server failed: %s', e)
