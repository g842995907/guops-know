...
class TrafficEventManager(object):
    ...
    def start(self, manual=False):
        ...
        if manual:
            ...
            delay_time = self.traffic_event.delay_time \
                if self.traffic_event.start_up_mode == TrafficEvent.StartUpMode.DELAY else 0
            delayed_message = {
                'channel': 'traffic',
                'content': data,
                'delay': delay_time * 1000 * 60
            }
            Channel('asgi.delay').send(delayed_message)
            self.logger.info("Channel send delay task, traffic [%s] minutes later", delay_time)
