...
class ImageAction(object):
    ...
    @property
    def glance_cli(self):
        if not hasattr(self, '_glance_cli'):
            self._glance_cli = gl_client()
        return self._glance_cli
    ...
    def delete_image(self, image_id):
        try:
            ...
            LOG.info("Image ({}) already deleted.".format(image_id))
            return True
        except Exception as e:
    ...
    @logger_decorator
    def check_image_status(self, image, type="Image", created=None, failed=None, timeout_callback=None):
