...
from sisdk.libcr.proto3.cr_topology_pb2 import topology_net_entity
from .proto2.cr_topology2_pb2 import topology_net_entity as topology_net_entity_2
...
class NetEntity(object):
    ...
    def to_binary_2(self):
        """
        为了兼容proto2
        :return:
        """
        if self.primary:
            primary_data = self.primary.to_binary_2()
        else:
            primary_data = None
        proto_data = topology_net_entity_2(id=self.id,
                                           name=self.name,
                                           parent_id=self.parent_id,
                                           is_primary=self.is_primary,
                                           primary=primary_data,
        ...
        if len(self.children) > 0:
            this_children = []
            for child in self.children:
                this_children.append(child.to_binary_2())
            proto_data.children.extend(this_children)
        return proto_data
...
class XojScene(object):
    ...
    def from_json(self, json_str):
        ...
        for sv in servers:
            ...
            # 有时候net对象是一个字典
            if len(sv['net']) == 0:
                obj_sv.network = 'dummy'
            elif isinstance(sv['net'][0], dict):
    ...
    def make_tree(self, root_id_or_name, auto_arrange_root=True):
        ...
        if not self.networks:
            ...
            for sv in self.servers:
                dummy_net.add_child(sv)
            dummy_root = Subnet("Root", "BIGNET")
            dummy_root.orientation = Orientation.automatic
            dummy_root_router = NetDevice("RT-Root", "RT-Root", EnumDeviceType.router)
            dummy_root.set_primary(dummy_root_router)
            self.root_entity = dummy_root
            self.root_entity.add_child(dummy_net)
            #self.root_entity = dummy_net
            return True
        else:
    ...
    def to_binary_2(self):
        if self.root_entity:
            return self.root_entity.to_binary_2()
