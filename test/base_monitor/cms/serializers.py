...
class MonitorCategorySerializer(ModelSerializer):
    en_name = serializers.CharField(min_length=2,
                                    max_length=100,
                                    validators=[UniqueValidator(queryset=monitor_models.MonitorCategory.objects.all())])
    cn_name = serializers.CharField(min_length=2,
                                    max_length=100,
                                    validators=[UniqueValidator(queryset=monitor_models.MonitorCategory.objects.all())])
...
class ScriptsSerializer(ModelSerializer):
    create_user_name = serializers.CharField(source='create_user.username', default='', read_only=True)
    device = StandardDeviceSerializer(source='checker', read_only=True)
    category_cn_name = serializers.CharField(source='category.cn_name', default='', read_only=True)
    category_en_name = serializers.CharField(source='category.en_name', default='', read_only=True)
    checker_name = serializers.CharField(source='checker.name', default='', read_only=True)
    ...
    def validate(self, attrs):
        ...
        if attrs.get("suffix") is None:
            raise exceptions.ValidationError({'suffix': [MonitorError.REQUIRED_FIELD]})
        ...
        if attrs.get("category") is None:
            raise exceptions.ValidationError({'category': [MonitorError.REQUIRED_FIELD]})
