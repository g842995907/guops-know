...
from sisdk.libcr.sandtable_generator import Sandtable_Generator
# 以下为了兼容proto2
from sisdk.libcr.packaged_messages_2 import PackagedMessages2
from sisdk.libcr.atom_sysctrl_2 import AtomSysctrl as AtomSysctrl2
from sisdk.libcr.atom_ui_2 import AtomUi as AtomUi2
...
class CpVisCr(CpVis):
    ...
    def cr_topology_init_scene(self, topo_data, root_entity, auto_arrange_root=True):
        ...
        if bin_msg2:
            ...
            logging.error("Generating proto2 version serialized topology error!")
            errors += 1
        return not bool(errors)

    def cr_sandtable_init_scene(self):
        #TODO:应该有一个参数用来描述沙盘数据
        msg = self.sg.to_binary()
        sandtable_data = self.sg.to_json()
        return self.pub_state(WORLD_STATES.INIT_SANDTABLE, sandtable_data, msg)
    ...
    def cr_topology_act_attack(self, src_obj, dest_obj, success=True, show_guildline=True,
        ...
        # 发送proto2兼容数据
        msg2 = PackagedMessages2.topo_attack(src_obj, dest_obj, success, show_guildline,
                                           show_panel, panel_text, panel_os,
                                           change_color, show_top_icon)
    ...
    def cr_topology_act_guideline(self, src_obj, dest_obj, color='00CCFF', duration=5):
        ...
        self.pub_message(msg)
        # 发送proto2兼容数据
        msg2 = PackagedMessages2.topo_guideline(src_obj, dest_obj, color, duration)
        self.pub_message(msg)
        return True
    ...
    def cr_topology_show_entity_panel(self, obj, os_name, panel_text):
        ...
        self.pub_message(msg)
        # 发送proto2兼容数据
        msg2 = PackagedMessages2.topo_show_entity_panel(obj, os_name, panel_text)
        self.pub_message(msg2)
        return True
    ...
    def cr_sysctrl_sync_timing_countdown(self, countdown_seconds=0):
        ...
        self.pub_state(WORLD_STATES.TIME_SECONDS_COUNTDOWN_PROTO2, saved_state, msg2)
        return True
