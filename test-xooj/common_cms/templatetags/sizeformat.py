...
@register.filter(name='change_state')
def change_state(values, state):
    if state == 'down':
        if values == 'enabled' or values == 'disabled':
            values = 'disabled'
        elif isinstance(values, int):
            values = '0'
        elif values.endswith('GB') or values.endswith('MB') or values.endswith('Bytes'):
            values = '0Bytes'
    return values
