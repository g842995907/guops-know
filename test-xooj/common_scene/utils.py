...
def generate_complex_str(length=12):
    ...
    complex_strs.extend([random.choice(string.digits) for i in range(dig_len)])
    complex_strs.extend([random.choice(string.ascii_uppercase) for i in range(upp_len)])
    # complex_strs.extend([random.choice(special_chars) for i in range(spe_len)])
    complex_strs.extend([random.choice(string.ascii_lowercase) for i in range(low_len)])
...
def report_resources(request_id, resources=None, err_msg=None):
    try:
        requests.post(api_settings.COMPLEX_MISC.get("report_resources"),
                      json={'env_id': request_id,
                            'resources': resources,
                            'error_messages': err_msg})
        LOG.info("Report resources . RequestId: {}".format(request_id))
    except Exception, e:
        LOG.error("Unable to report resources to oj {}. {}".format(request_id, e))


def report_status(request_id, type, resource_id, status):
    try:
        report_url = api_settings.COMPLEX_MISC.get("report_{}_status".format(type))
        data = {'resource_id': resource_id, 'status': status}
        if request_id:
            data.update({'request_id': request_id})
        LOG.debug("Report url: {}, data: {}".format(report_url, data))
        requests.post(report_url, data=data)
        LOG.info("Report resource {} status {} to oj.".format(resource_id, status))
    except Exception, e:
        LOG.error("Unable to report resource {} status to oj. {}".format(resource_id, e))


def report_image_status(resource_id, status):
    try:
        report_url = api_settings.OFFLINE_MISC.get("report_image_status")
        data = {'image_id': resource_id, 'state': status}
        LOG.debug("Report url: {}, data: {}".format(report_url, data))
        requests.post(report_url, data=data)
        LOG.info("Report resource {} status {} to oj.".format(resource_id, status))
    except Exception, e:
...
def report_vm_status(resource_id, status):
    try:
        report_url = api_settings.OFFLINE_MISC.get("report_vm_status")
        data = {'instance_id': resource_id, 'state': status}
        LOG.debug("Report url: {}, data: {}".format(report_url, data))
        requests.post(report_url, data=data)
        LOG.info("Report resource {} status {} to oj.".format(resource_id, status))
    except Exception, e:
...
def address_net_bit(ip_addr):
    return ".".join(ip_addr.split(".")[:-1])


def address_cidr(ip_addr):
    LOG.info("Generate cidr from ip address {}".format(ip_addr))
    net_bits = ip_addr.split(".")[:-1]
    net_bits.append("0/24")
    return ".".join(net_bits)
...
def _acquire_lock(mc, key, attempts, expires):
    for i in xrange(0, attempts):
        ...
        if i != attempts-1:
            ...
            logging.debug('Sleeping for %ss while trying to acquire key %s', sleep_time, key)
...
def logger_decorator(logger):
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            logger.debug("[OPENSTACK] Start {}(): args={}, kwargs={}".format(func_name,
                                                              args, kwargs))
