...
class SessionViewSet(PMixin, viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        add_operate_log(user, module.SYSTEM,
                        'name[{}] login system, login user name [{}]'.format(user.username, user.name or user.username), 0,
                        _('x_system_login').format(user.name or user.username),)
...
class UserViewSet(BatchSetModelMixin, DestroyModelMixin, CacheModelMixin, PMixin, viewsets.ModelViewSet):
    ...
    def batch_destroy(self, request):
        ...
        if request.user.id in ids:
            raise exceptions.PermissionDenied(error.PERMISSION_NOT_ALLOWED)
        return super(UserViewSet, self).batch_destroy(request)
