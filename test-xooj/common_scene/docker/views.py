...
class ContainerAction(object):
    ...
    @property
    def zun_cli(self):
        if not hasattr(self, '_zun_cli'):
            self._zun_cli = zun_client()
        return self._zun_cli
    ...
    def delete_container(self, container_id, sync=True, force=True):
        try:
            ...
            LOG.info("Container ({}) already deleted.".format(container_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get container ({}). \n{}".format(container_id, e))
                attempts -= 1
                time.sleep(1)
        return True
