...
class StandardDeviceViewSet(RecordMixin, BatchSetOwnerModelMixin, DestroyModelMixin, CacheModelMixin, PMixin, viewsets.ModelViewSet):
    ...
    def sub_perform_update(self, serializer):
        ...
        if upload_mode:
            if StandardDeviceSnapshot.objects.filter(standard_device=instance).exists():
        ...
        if upload_mode:
            ...
            result = self._try_upload_image(validated_data.get('name') or instance.name,
                                            ...
                                            validated_data.get('disk_format') or instance.disk_format, meta_data)
            # 未上传则更新元数据
