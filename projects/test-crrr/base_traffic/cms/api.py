...
from base.utils.rest.mixins import CacheModelMixin, PMixin, DestroyModelMixin, RecordMixin
...
class TrafficViewSet(RecordMixin, DestroyModelMixin, CacheModelMixin, PMixin, viewsets.ModelViewSet):
...
class IntelligentTrafficViewSet(TrafficViewSet):
    ...
    def sub_perform_create(self, serializer):
        with transaction.atomic():
            ...
            if not self.request.data.get('tgm'):
            ...
            file_path = SCRIPT_URL + traffic.hash.split('.')[0] + script_type
