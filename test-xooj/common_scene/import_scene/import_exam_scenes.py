...
from utils import ComposeYamlV2, YamlConvert, \
...
def main(course_name, data_yaml_path, legal_type):
    course_base_dir = os.path.split(data_yaml_path)[0]

    yc = YamlConvert(data_yaml_path)
    course_env_dict = {}
    env_scene_dict = {}
    one_opt_scene_dict = {}
    tasks_count = 0
    ...
    yml_root_name, yml_datas = yc.content.items()[0]
    for type, tasks in yml_datas.items():
        if type.lower() not in legal_type:
            print "type({}) not in LEGAL_TYPES".format(type)
            continue
        tasks_count += len(tasks)
        for task in tasks:
            task_name, task_data = task.items()[0]
            if isinstance(task_data, list):
                task_data = convert_dict_list(task_data)
            env = task_data.get("env", "")
            if not env:
                print "task ({}) env is None, skip".format(task_name)
                continue
            # env_abspath = os.path.abspath(os.path.join(course_base_dir, env))
            scene_name = "{}-{}-{}".format(course_name, type, task_name)

            # only operator not none
            opt = task_data.get("operator")
            if env.lower() == "none" and opt and opt.lower() != "none":
                env = "{}-{}".format(env, uuid.uuid4().hex[:6])
                print "Just operator for course {}".format(scene_name)
                try:
                    if opt in one_opt_scene_dict.keys():
                        ...
                        scene = convert_one_operator(scene_name, **task_data)
                        env_scene_dict.update({env: scene.get("id")})
                        one_opt_scene_dict.update({opt: scene})
                    course_env_dict.update({scene_name: scene.get("id")})
                except Exception, e:
                    print "ERR: Unable to create env {} : {}".format(env, e)
                    error_envs.append(scene_name)
                continue
            ...
            # env is a file
            if not env.endswith(".yml") and os.path.isfile(os.path.join(course_base_dir, env)):
            ...
            if env in created_envs:
                ...
                repeat_count += 1
                course_env_dict.update({scene_name: env_scene_dict.get(env)})
                continue

            try:
                # create a new scene
                scene = convert_one_v2(course_base_dir, scene_name, **task_data)
    ...
    print "All scene created for {} \n" \
          ...
          "error count: {} \n".format(course_base_dir,
                                      tasks_count, len(course_env_dict),
...
if __name__ == "__main__":
    ...
    task_folders = [sys.argv[1]]
    legal_type = sys.argv[2:] or ["crypto", "pwn", "web", "misc", "reverse", "mobile", "ppc", "network"]
    # task_folders = ["/home/moose/code/x-oj/common_scene/docker/compose/getting_start"]
    # legal_type = ["web"]

    for task_folder in task_folders:
        if not os.path.exists(task_folder):
            print "Folder not exists () ".format(task_folder)
            exit(1)
        ...
        if task_folder.startswith(current_dir):
            fld_name = task_folder.split(current_dir)[-1]
            if fld_name.startswith("/"):
                fld_name = fld_name[1:].replace("/", "_")
        else:
            _, fld_name = os.path.split(task_folder)

        data_yaml_path = os.path.join(task_folder, "data.yml")
        if os.path.exists(data_yaml_path):
            success_course, error_course = main(fld_name, data_yaml_path, legal_type)
            succ_course_env_dict.update({fld_name: success_course})
            err_course_env_dict.update({fld_name: error_course})
        else:
            for fld in os.listdir(task_folder):
                data_yaml_path = os.path.join(task_folder, fld.decode("utf-8"), "data.yml")
                if not os.path.exists(data_yaml_path):
                    print "WARN: No data.yml found : {}, skip".format(data_yaml_path)
                    continue
                success_course, error_course = main(fld, data_yaml_path, legal_type)
                succ_course_env_dict.update({fld: success_course})
                err_course_env_dict.update({fld: error_course})

        with open("./practice-success-{}.json".format(fld_name), 'w') as f:
            f.write(json.dumps(succ_course_env_dict, indent=4))

        with open("./practice-error-{}.json".format(fld_name), 'w') as f:
