...
class CrEventViewSet(BatchSetOwnerModelMixin,
    ...
    def get_retrieve_cache_key(self):
        ...
        main = self.request.query_params.get('main', '')
        return '{0}--{1}--{2}--{3}--retrieve-USER{4}'.format(view_name, self.lookup_field, retrieve_key, main,
    ...
    def get_queryset(self):
        ...
        if event_status is not None and event_status:
            ...
            if int(event_status) == self.EVENTSTATUS.NOTSTARTED:
                queryset = queryset.filter(start_time__gt=now)
            elif int(event_status) == self.EVENTSTATUS.PROCESSING:
                queryset = queryset.filter(Q(start_time__lte=now) & Q(end_time__gte=now))
            elif int(event_status) == self.EVENTSTATUS.HASENDED:
    ...
    def sub_perform_create(self, serializer):
        ...
        if scene_models.CrEvent.objects.filter(name=validated_data['name']).exists():
    ...
    def sub_perform_update(self, serializer):
        ...
        if 'name' in validated_data:
            if scene_models.CrEvent.objects.exclude(pk=serializer.instance.pk).filter(
    ...
    def get_team_score(self, request, pk):
        ...
        for cr_scene in crscene_objs:
            ...
            missions = cr_scene.missions.all()
            missions_ids = [mission.id for mission in missions]
            missions_count = len(missions_ids)

            answer_list = scene_models.CrSceneEventUserAnswer.objects.filter(cr_event=obj,
                                                                             mission_id__in=missions_ids) \
                .values('score')
            mission_sove_count = len(answer_list)
            mission_data_dict['score'] = sum([float(item['score']) for item in answer_list])
            mission_data_dict['cr_scene_id'] = cr_scene.id
            mission_data_dict['missions_count'] = missions_count
            mission_data_dict['mission_sove_count'] = mission_sove_count
    ...
    def control_mission_status(self, request, pk):
        ...
        if mission_status == constant.MissionStatus.ISPROCESS:
            SceneMissionManager(cr_scene_instance, True).manual_start_mission_check(mission_id)
        elif mission_status == constant.MissionStatus.STOP:
            SceneMissionManager(cr_scene_instance, True).manual_stop_mission_check(mission_id)
    ...
    def event_report(self, request, pk):
        ...
        # todo 需要考虑 checker题目的提交，不在一个日志表里面， 和下次的渲染数据一起完善
        # crscenemissionchecklog = scene_models.CrSceneMissionCheckLog.objects.filter(cr_event=obj)
    ...
    if scene_id:
        ...
        if status == Scene.Status.RUNNING:
            if event == SceneStatusUpdateEvent.SCENE_CREATE:
                ...
                # 流量发生开始
                SceneTrafficManager(scene_id, cr_event_id=cr_event_scene_id).start_traffic_event()

                # 检测任务开始
                SceneMissionManager(scene_id, True).start_mission_check()
        elif status == Scene.Status.DELETED:
            # 删除流量
            SceneTrafficManager(scene_id, cr_event_id=cr_event_scene_id).stop_traffic_event()

            # 删除检测任务
            SceneMissionManager(cr_event_scene_id, True).stop_mission_check()
        else:
            pass
...
class AgentViewSet(CacheModelMixin,
    ...
    def get_queryset(self):
        ...
        machine_id = self.query_data.get('machine_id', int)
        if machine_id:
            queryset = queryset.filter(machine_id=machine_id)

        timedelta = self.query_data.get('timedelta', int)
        if timedelta:
            start_time = datetime.datetime.now() - datetime.timedelta(hours=int(timedelta))
