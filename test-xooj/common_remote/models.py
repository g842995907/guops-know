...
class GuacamoleUserPermission(models.Model):
    user = models.ForeignKey(GuacamoleUser, on_delete=models.CASCADE, related_name="user")
    affected_user = models.ForeignKey(GuacamoleUser, on_delete=models.CASCADE, related_name="affected_user")
    Permission = enum(
        READ='READ',
        UPDATE='UPDATE',
        DELETE='DELETE',
...
# 视频转换待完成任务
class RecordingConvertTask(ExecutePool):
    pass