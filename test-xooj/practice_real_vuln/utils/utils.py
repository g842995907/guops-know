...
def handle_markdown(path):
    ...
    copy_path = os.path.join(settings.MEDIA_ROOT, 'practice/realvuln/markdown_img')
    relative_copy_path = '/media/practice/realvuln/markdown_img/'
    format_true = False
    ...
    for each_file in path_dir:
        ...
        if each_file.split('.')[-1] == 'md':
            ...
            with codecs.open(each_file_path, 'r', 'utf-8') as mdFile:
                html_str_content = markdown.markdown(mdFile.read(), extensions=exts,                                                    extension_configs={
                                                         'markdown.extensions.codehilite': {
                                                             'configs':{
                                                             'linenums': False}
                                                         }
