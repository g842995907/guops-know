...
from neutronclient.common.exceptions import NetworkNotFoundClient, PortNotFoundClient
...
class NetworkAction(object):
    ...
    def neutron_cli(self):
        if not hasattr(self, '_neutron_cli'):
            self._neutron_cli = nt_client()
        return self._neutron_cli
    ...
    def delete_fip(self, fip_id, sync=True):
        try:
            ...
            LOG.info("Floating ip ({}) already deleted.".format(fip_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get floating ip ({}). \n{}".format(fip_id, e))
                attempts -= 1
                time.sleep(1)
        return True
    ...
    @logger_decorator
    def delete_port(self, port_id, sync=True):
        try:
            self.neutron_cli.port_delete(port_id)
        except PortNotFoundClient:
            LOG.info("Port ({}) already deleted.".format(port_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    self.neutron_cli.port_get(port_id)
                except PortNotFoundClient:
                    LOG.info("Deleted port {}".format(port_id))
                    break
                except Exception as e:
                    LOG.debug("Failed to get port ({}). \n{}".format(port_id, e))
                attempts -= 1
                time.sleep(1)
        return True
    ...
    def delete_network(self, network_id, sync=True):
        try:
            ...
            LOG.info("Network ({}) already deleted.".format(network_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get network ({}). \n{}".format(network_id, e))
                attempts -= 1
                time.sleep(1)
    ...
    def delete_subnet(self, subnet_id, sync=True):
        try:
            ...
            LOG.info("Subnet ({}) already deleted.".format(subnet_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get subnet ({}). \n{}".format(subnet_id, e))
                attempts -= 1
                time.sleep(1)
    ...
    def delete_router(self, router_id, sync=True):
        try:
            ...
            LOG.info("Router ({}) already deleted.".format(router_id))
            return True
        except Exception as e:
        ...
        if sync:
            ...
            while attempts:
                try:
                    ...
                    LOG.debug("Unable to get router ({}). \n{}".format(router_id, e))
                attempts -= 1
                time.sleep(1)
    ...
    def delete_firewall_rule(self, rule_id, is_async=True):
        try:
            ...
            LOG.info("Firewall rule ({}) already deleted.".format(rule_id))
        except Exception as e:
    ...
    def delete_firewall_policy(self, policy_id):
        try:
            ...
            LOG.info("Firewall policy ({}) already deleted.".format(policy_id))
        except Exception as e:
    ...
    def delete_firewall(self, firewall_id):
        try:
            ...
            LOG.info("Firewall ({}) already deleted.".format(firewall_id))
        except Exception as e:
