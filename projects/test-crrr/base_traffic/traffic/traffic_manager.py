...
from base_traffic.traffic.constants import DelayTime, MAX_CONNECTIONS, MAX_MANUAL_CONNECTIONS
...
class BaseTraffic(object):
    def __init__(self, data, traffic_event, manual=False):
        self.data = data
        self.scene_id = data.get('scene_id')
        self.traffic_event = traffic_event
        self.logger = TrafficLogFactory(self.scene_id, __name__)
        self.manual = manual
        self.max_connections = MAX_MANUAL_CONNECTIONS if self.manual else MAX_CONNECTIONS
        self.trial_times = 0
    ...
    def trail_connect(self, message):
        if message.get('status') == 'down':
            if self.trial_times <= self.max_connections:
                self.logger.info("Traffic[%s]: try to connect [%d/%d]", self.traffic_event.title, self.trial_times,
