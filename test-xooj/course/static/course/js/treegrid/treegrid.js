...
(function ($) {
    ...
    var getParent = function (node, source, field) {
        ...
        var items = $.grep(source, function (item, index) {
            return node.parent_id == item[field];
        });
    ...
    var getChild = function (node, source, field) {
        var items = $.grep(source, function (item, index) {
            return item.parent_id == node[field];
        });
        return items;
    };
                ...
                value = typeof value === 'undefined' || value === null ?
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        ...
                        var child = $.grep(data, function (d, i) {
                            return d.parent_id == item[that.options.treeId] && !d.hidden;
                        });
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        // 自定义展开,收起按钮
        this.$body.find('.tree-expand').off('click').on('click', function (e) {
            e.stopPropagation();
            var $this = $(this),
            ...
            var icon = $tr.find('td span.tree-icon');
            if(icon.hasClass(that.options.expandIcon)){
                //展开状态
                row['expand'] = false;
                icon.removeClass(that.options.expandIcon).addClass(that.options.collapseIcon);
        ...
        $('.check_remove').parent().parent().find('td:first div').removeClass('checkbox');
        $('.check_remove').parent().parent().find('td:first div input').remove();
        $('.blank_replace').html('&nbsp;&nbsp;&nbsp;&nbsp;')
