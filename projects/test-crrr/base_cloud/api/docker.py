...
from base.utils.network import cport
...
class Docker(object):
    ...
    def local_create(self, **kwargs):
        ...
        subprocess.call('{docker} run --name {name} -itd {port_str} {image}'.format(
...
def random_docker_host(tool=None):
    if tool is None:
        tool = copy.copy(app_settings.RESOURCE_HOSTS)

    if not tool:
        raise Exception('no valid docker host')

    host = random.choice(tool)
    if host and not cport(host, 2375):
        tool.remove(host)
