...
class ControlMixin(object):
    ...
    def save_image(self, image_name, created=None, failed=None):
        ...
        # 删除旧的镜像
        try:
            old_image_id = cloud.image.get(image_name=image_name).id
        except Exception:
            old_image_id = None
        ...
        def image_created(image):
            ...
            if old_image_id:
                try:
                    cloud.image.delete(image_id=old_image_id)
                except Exception as e:
                    logger.error('delete old image[%s] error: %s', old_image_id, e)
...
class CreateMixin(object):
    ...
    def local_create_server(self):
        ...
        return {
            ...
            'host_ip': server.host_ip_address,
        }
...
def get_terminal_network_id(scene_terminal):
    scene_net = scene_terminal.nets.first()
    if scene_net:
