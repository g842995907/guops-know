...
from practice_real_vuln.setting import api_settings
...
def task_list(request, event_id, **kwargs):
    type_id = practice_api.PRACTICE_TYPE_REAL_VULN
    event_id = event_id
    context = {
...
def do_task(request, **kwargs):
    context = kwargs.get('menu')
    return render(request, 'practice_real_vuln/web/do_task.html', context)
