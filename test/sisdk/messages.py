...
from sisdk.libcr.proto3.base_pb2 import sequence_message, atom_message
from sisdk.libcr.proto2.base2_pb2 import sequence_message as sequence_message_2
from sisdk.libcr.proto2.base2_pb2 import atom_message as atom_message_2


from sisdk.libcr.proto3.cr_sysctrl_pb2 import sysctrl_scoreboard, req_click
...
class SequenceMessageMaker(object):
    ...
    def __init__(self, message_type=EnumMessageType.behavior, scenario_type=EnumScenarioType.all_scenarios):
        ...
        self.instance.header.server = "127.0.0.1"   # 暂未使用
        self.instance.header.channel = "6"          # 暂未使用
...
class SequenceMessageMaker2(object):
    """
    用于兼容proto2格式的smq类
    """
    def __init__(self, message_type=EnumMessageType.behavior, scenario_type=EnumScenarioType.all_scenarios):
        ...
        :param scenario_type: 场景类型
        """
        self.instance = sequence_message_2()
        self.instance.header.id = str(time.time() * 1000)
        self.instance.header.timestamp = int(time.time() * 1000)
        self.instance.header.server = "127.0.0.1"   # 暂未使用
        self.instance.header.channel = "6"          # 暂未使用
    ...
    def add_fragment(self, frag_message, wait=0):
        ...
        atom.meta.wait = wait
        try:
            atom.content = base64.b64encode(frag_message.SerializeToString())
        except Exception as e:
            atom.content = base64.b64encode("Cannot convert serialized object: " + str(e))
        self.atoms.append(atom)
...
class ScoreboardMessage(object):
    ...
    def to_binary(self):
        score_infos = []
        for entry in self.entries:
            scoreboard_info = sysctrl_scoreboard.msg_score_info(name=entry.get("name"),
        ...
        scoreboard_msg = sysctrl_scoreboard(title=self.title)
...
def wrap_message_2(message):
    smq = SequenceMessageMaker2(EnumMessageType.command, EnumScenarioType.all_scenarios)
    smq.add_fragment(message)
    return smq.serialized()
